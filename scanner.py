...
def phase_subdomains(domain, session_dir):
    ...
    if tool_exists("subfinder"):
        ...
        out, _, _ = run_cmd(["subfinder", "-d", domain, "-silent", "-all"], timeout=180)
    ...
    if tool_exists("subdominator"):
        ...
        run_cmd(["subdominator", "-d", domain, "-o", str(sub_out_file), "-silent"], timeout=180)
...
def phase_ports(resolved_file, session_dir, port_range=None):
    ...
    if tool_exists("naabu"):
        run_cmd(
            ["naabu", "-list", str(resolved_file), "-p", str(port_range), "-silent", "-json", "-o", str(ports_json)],
            timeout=600
        )
        out_plain, _, _ = run_cmd(
            ["naabu", "-list", str(resolved_file), "-p", str(port_range), "-silent"],
            timeout=600
        )
...
def phase_tech_detection(resolved_file, session_dir):
    ...
    if tool_exists("httpx"):
        run_cmd(
            ["httpx", "-l", str(resolved_file), "-silent", "-status-code", "-title", "-tech-detect", "-json", "-o", str(httpx_json)],
            timeout=300
        )
...
def phase_urls(domain, live_hosts, session_dir):
    ...
    if tool_exists("katana") and live_hosts:
        ...
        run_cmd(
            ["katana", "-list", str(roots_file), "-d", "2", "-silent", "-o", str(katana_file)],
            timeout=300
        )
...
def phase_vuln_scan(resolved_file, session_dir):
    ...
    if tool_exists("nuclei"):
        run_cmd(
            ["nuclei", "-l", str(resolved_file), "-silent", "-severity", "low,medium,high,critical", "-tags", "subdomain-takeover,cves,exposures,misconfiguration", "-json", "-o", str(vuln_json)],
            timeout=600
        )
        if vuln_json.exists():
